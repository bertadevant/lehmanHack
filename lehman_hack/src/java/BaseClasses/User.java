...
public class User {
	...
	public int countVote(boolean flag, String id, int direction) {
        DBConnect connect = new DBConnect();
        connect.createStatement();
        String query = "";
        if (flag) {
            query = "SELECT * FROM lehman_hack.ThreadVotes WHERE Thread_idThread = '" + id + "'AND type = " + direction;
        } else {
            query = "SELECT * FROM lehman_hack.CommentVotes WHERE Comment_idComment = '" + id + "'AND type = " + direction;
        }
        System.out.println(query);
        connect.querySelect(query);
        int count = 0;
