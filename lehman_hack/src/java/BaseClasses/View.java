...
public class View {
    ...
    public List comment(int threadId) {
        ...
        try {
            while (connect.getResultset().next()) {
                ...
                Comment newComment = new Comment(id,  un, cComment, cData);
    ...
    public String[] thread(int threadId) {
        ...
        //thread[2] -> date
        String[] thread = new String[4];
        DBConnect connect = new DBConnect();
        connect.createStatement();
        String query = "SELECT title, post, date, User_username	 FROM `Thread` WHERE idThread = " + threadId;
        System.out.println(query);
        connect.querySelect(query);
        ...
        try {
            if (connect.getResultset().next()) {
                ...
                String da = connect.getResultset().getString("date");
                String username = connect.getResultset().getString("User_username");
                thread[0] = ti;
                thread[1] = description;
                thread[2] = da;
                thread[3] = username;
